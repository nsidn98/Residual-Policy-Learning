...
mpirun python -m RL.ddpg_mpi --env_name=$env_names --exp_name='res' --beta=0.005 --beta_monitor='actor' --n_epochs 500 2>&1 | tee $env_names/out_actor
# this one is to check if residual learning happens by monitoring critic loss for burn-in
mpirun python -m RL.ddpg_mpi --env_name=$env_names --exp_name='res' --beta=0.005 --beta_monitor='critic' --n_epochs 500 2>&1 | tee $env_names/out_critic