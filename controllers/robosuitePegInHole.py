...
class keypointOptPegInHole(gym.Env):
    ...
    def __init__(self, *args, **kwargs):
        ...
        qRob0RelPeg = T.quat_multiply(T.quat_conjugate(observation["peg_quat"]),observation["robot0_eef_quat"])
        qRob1RelHole = T.quat_multiply(T.quat_conjugate(observation["hole_quat"]),observation["robot1_eef_quat"])
        ...
        q_rob0_2 = T.quat_multiply(q_peg_2,qRob0RelPeg)
    ...
    def controller(self,obs:dict):
        ...
        if self.poseNum == 0:
            ...
            # axangActionRob0 = self.kpp*(T.quat2axisangle(T.quat_multiply(T.quat_conjugate(obs["robot0_eef_quat"]),T.axisangle2quat(self.axang_rob0_1))))
            # axangActionRob1 = self.kpp*(T.quat2axisangle(T.quat_multiply(T.quat_conjugate(obs["robot1_eef_quat"]),T.axisangle2quat(self.axang_rob1_1))))
    ...
    def cons_1(self,x):
        ...
        q_peg = x[6:10]
        q_hole = x[10:14]
        p_peg_in_hole_frame = np.matmul(T.quat2mat(q_hole),p_peg - p_hole)
